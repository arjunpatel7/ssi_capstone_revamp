...
#found a mis/type for sanfrancisco
unemp_df[grep("San Francisco", unemp_df$Area_name), c("Area_name")] = "san francisco"
...
unemp_df = unemp_df %>% gather("unemployment_year", "percent_unemployed", colnames(unemp_df)[2:7])
unemp_df$unemployment_year = gsub("unemp", "", unemp_df$unemployment_year)
...
for(x in colnames(cali_pop_data)[4:14]){
  ...
  cali_pop_data[, x] = (cali_pop_data[, x]/(cali_pop_data$`All races`)) * 100
}

cali_pop_data$NAME = str_to_lower(gsub(" County, California", "", cali_pop_data$NAME))




df = merge(unemp_df, cali_pop_data, by.x = "county", by.y = "NAME")

length(unique(cali_pop_data$NAME)) ==length(unique(unemp_df$county))

df = merge(df, cali_hc_df, by.x = c("county", "unemployment_year"), by.y = c("County.y", "ClosedYear"))
df = merge(mobile_internet_df, df,  by.x = c("county", "year"), by.y = c("county", "unemployment_year"))
...
plot(hc_capita ~ percent_unemployed, data = df)
...
cor(df[, c("hc_capita")], df[, c("percent_unemployed")])

m1 = lm(hc_capita ~ wtcp_down_mean  + log10(`Black alone`), data = df %>% filter(hc_capita !=0))
summary(m1)

plot(cooks.distance(m1))


m_fixed_eff <- lm(hc_capita ~ log10(wtcp_down_mean)+ log10(`Black alone`) +
                    log10(`Asian alone`) + percent_unemployed + as.factor(year) + log10(`White alone`), data = df)
summary(m_fixed_eff)
